...
object DomainBlocklist:
  ...
  private val SocialNetworks = Set(
    ...
    "threads.net",
    "bsky.app",
    "mastodon.social", // covers one instance; operators may want per-instance config
    "vk.com",
    "weibo.com",
    "discord.com",
    "discord.gg",
  ...
  private val AppStores = Set(
    ...
    "microsoft.com/store", // note: domain-level match won't work for paths;
    // microsoft.com is too broad to block entirely.
    // Rely on the permissive filter for this one.
    "galaxy.store"
  )
